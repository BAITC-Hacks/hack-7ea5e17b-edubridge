...
**Февральский replay выполнен:** 29 реальных архивных выпусков на 48 часов, 2 784 записи полных горизонтов, выбранные **1 344 строки — 672 часа на каждую турбину**, без пропусков. Готовые результаты: [CSV](outputs/forecast/february-2026-48h.csv), [JSON](outputs/forecast/february-2026-48h.json), [аудит и SHA256](docs/evidence/february-replay.json). Проверены временная доступность погоды, cutoff модели и содержимое 1 392 сырых погодных пакетов. Фактической выработки февраля нет, поэтому это подтверждение воспроизводимого расчёта, а не измеренная февральская точность.
...
**Чистая установка интеграции:** отдельный checkout с зафиксированными хешами кода, новая venv без переноса cache/артефактов, установка всей команды из README, `pip check`, **85 тестов API/evaluation/UI**, реальный inference сохранённой модели на архивном sample и запуск обоих сервисов прошли. [Отчёт чистой установки](docs/evidence/integration-clean-install.json). Ранее отдельно проверялась платформа: [исторический отчёт](docs/evidence/platform-validation.json).

Workflow настроен на тесты, lint и синтетический replay, однако GitHub Actions **не смог запустить ни одного шага из-за billing-блокировки аккаунта**; успешный CI или Linux-проверку не заявляем.
