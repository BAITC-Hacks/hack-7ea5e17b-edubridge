...
Тесты без сети: поздняя публикация, координаты, cutoff, полнота часов, retry/cache, multipart, ошибки провайдера/модели, идемпотентность, ревизии, границы февраля/марта, API/CLI. Реальные запросы отделены в probe. CI выполняет тесты, lint и **синтетический** replay; сетевую доступность архива CI не доказывает.
